...
const Page = (props) => {
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.tectonicSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.tectonic;
  })
	...
	return(
      ...
      <Box sx={{display: 'flex', flexDirection: 'column',marginTop:'48px'}}>
        ...
        <img className='grill_picture' src="./assets/img/croge-grill.svg" alt='tectonic' />
      ...
      <Card grill="TONIC" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 3% of the TONIC used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
					<Link to='https://cronoscan.com/address/0x08137Ef8a7CBaFcbaDA55073a1be3B995B436365' target="_blank"><img src="./1.png" alt='address'/></Link>
