...
const Page = (props) => {
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.mmfSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.mmf;
  })
  ...
  // const {img} = props;
	return(
      ...
      <Card grill="MMF" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 8% of the MMF used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
