...
const Page = (props) => {
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.vvsSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.vvs;
  })
  ...
  // const {img} = props;
	return(
      ...
      <Card grill="VVS" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 3% of the VVS used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
					<Link to='https://cronoscan.com/address/0x90BF094D2458805966b4F8B51804fC7299b6d4bb' target="_blank"><img src="./1.png" alt='address'/></Link>
