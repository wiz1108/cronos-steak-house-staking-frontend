...
const Page = (props) => {
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.svnSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.svn;
  })
  ...
  // const {img} = props;
	return(
      ...
      <Card grill="SVN" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 8% of the SVN used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
