...
const Page = (props) => {
	...
	let query = useQuery();
	let ref = query.get('ref') || '0x4EC8a93Fe8c53C61f192DFb136aaA2cD7401a408';
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.crogeSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.croge;
  })
	...
	const buyBeans = async (amount) => {
		...
		try {
			const cost = ethers.utils.parseUnits(amount, 9);
	...
	return(
      ...
      <Box sx={{display: 'flex', flexDirection: 'column',marginTop:'48px'}}>
        ...
        <img className='grill_picture' src="./assets/img/croge-grill.svg" alt='croge' />
      ...
      <Card grill="CROGE" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Fact">
          <div style={{padding: '12px 24px'}}>
            ...
            <div className="m_between" style={{marginTop:'20px'}}>
              ...
              <div className="Fact_font2">up to 9%</div>
            </div>
            ...
            <div className="m_between">
              ...
              <div className="Fact_font2">3,285%</div>
            </div>
          </div>
        </div>
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 3% of the CROGE used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
					<Link to='https://cronoscan.com/address/0xf7E010D1B831DeD375E54edF3771eD2627D2CC5C' target="_blank"><img src="./1.png" alt='address'/></Link>
