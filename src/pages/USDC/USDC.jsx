...
const Page = (props) => {
  ...
  const [chefs, setChefs] = useState(0);
	...
	const steakContract = useSelector((state) => {
		return state.user.usdcSteak;
	});

  const tokenContract = useSelector((state) => {
    return state.user.usdc;
  })
	...
	const buyBeans = async (amount) => {
		...
		try {
			const cost = ethers.utils.parseUnits(amount, 6);
	...
	async function fetchData() {
      ...
      steakContract.getBalance().then(balance => {
				setContractBalance(ethers.utils.formatUnits(balance, 6));
			});
      await tokenContract.balanceOf(walletAddress).then(balance => {
				setWalletBalance(ethers.utils.formatUnits(balance, 6));
			});
      steakContract.getInvestorCount().then(_chefs => {
				setChefs(_chefs.toString());
			});
			steakContract.getMyChefs(walletAddress).then(amount => {
				setSteak(ethers.utils.formatUnits(amount, 4))
			});
			steakContract.steakRewards(walletAddress).then(amount => {
				setReward(ethers.utils.formatUnits(amount, 6));
			});
		}
	}
	useEffect(() => {
  ...
  // const {img} = props;
	return(
      ...
      <Card grill="USDC" poolBalance={contractBalance} walletBalance={walletBalance} steak={steak} reward={reward} chefs={chefs}
        ...
        <div className="Link">
          <div style={{padding:'12px 24px'}}>
            ...
            <div className="Link_font2">
              Earn 5% of the USDC used to grill steak from anyone who uses your referral link
            </div>
          </div>
        </div>
        <div className="Icon">
